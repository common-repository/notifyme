...
/*
Plugin Name: NotifyMe
Plugin URI: http://www.iamklaus.org/notify
...
register_activation_hook( __FILE__, 'notifyme_create_db' );
function notifyme_create_db() {

    global $wpdb;
    $charset_collate = $wpdb->get_charset_collate();
    $table_name = $wpdb->prefix . 'notifyme';
...
function ajax_enqueue_scripts() {
	...
	wp_enqueue_script( 'notifyme', plugins_url( '/js/notifyme.js', __FILE__ ), array('jquery'), '1.0', true );
	wp_localize_script( 'notifyme', 'postnotifyme', array( 'ajax_url' => admin_url( 'admin-ajax.php' )	));
...
function post_notifyme_display ( $content ) {    
    ...
    if($result_count > 0) {
	    $content .= '<a class="notifyme-button" id="notifyme" href="' 
	        . admin_url( 'admin-ajax.php?action=post_notifyme_add_notifyme&post_id='
	        . get_the_ID() ) 
...
add_action( 'wp_ajax_nopriv_post_notifyme_add_notifyme', 'post_notifyme_add_notifyme' );
add_action( 'wp_ajax_post_notifyme_add_notifyme', 'post_notifyme_add_notifyme' );
function post_notifyme_add_notifyme() {
...
add_action( 'post_updated', 'notifyme', 10, 3 ); 
function notifyme($post_id, $post_after, $post_before){
    ...
    if ( $current_user = wp_get_current_user() ) {
        $message .= '<br><br>';
        $message .= 'Modified by: <a href="'. get_author_posts_url($current_user->ID, $current_user->display_name). '">'. $current_user->display_name . '</a>';
    }
